...
alias_codex: ## add cx alias to run codex
	@if ! grep -q "alias cx=" ~/.zshrc; then \
		echo "alias cx='docker run --rm -it -v \$$(pwd):/app -v \$$(pwd)/.git:/app/.git:ro -v ~/.codex:/root/.codex -v /root/.codex/packages -e GOPROXY=\"https://nexus.skyunion.net/repository/go-group,https://proxy.golang.org,direct\" -e GONOSUMDB=\"git.skyunion.net/*\" $(CODEX_NAME):$(TAG)'" >> ~/.zshrc; \
		echo "✅ 已成功加入 alias"; \
	else \
